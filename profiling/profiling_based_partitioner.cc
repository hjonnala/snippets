...
int64_t ProfilingBasedPartitioner::PartitionOnTargetLatency(
  ...
  const auto first_segment_latency = PartitionCompileAndAnalyze(
      num_ops, num_segments_ - 2, delegate_search_step);
  ...
  LOG(INFO) << "target_latency: " << target_latency
            ...
            << ", first segment latency: " << first_segment_latency;
  partition_ = num_ops;
  return std::abs(last_segment_latency - first_segment_latency);
}
...
bool BisectPartition(const std::string& edgetpu_compiler_binary,
  ...
  while (std::abs(upper_bound - lower_bound) > diff_threshold_ns) {
    ...
    const int64_t fast_slow_latency_diff = partitioner.PartitionOnTargetLatency(
        target_latency, partition_search_step, delegate_search_step);
    if (fast_slow_latency_diff <= diff_threshold_ns){
...
//    if last_segment_latency >= diff_threshold_ns:
//        best_partition = partitioner.partition();
//        CHECK(!best_partition.empty());
//        break;

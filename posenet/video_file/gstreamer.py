...
def run_pipeline(inf_callback, render_callback, src_size,
    ...
    elif '.MP4' in videosrc.upper():
        #PIPELINE = f'filesrc location={videosrc} ! decodebin ! videoconvert ! video/x-raw,format=RGB ! videoconvert ! autovideosink  demux.video_0 ! queue'
        #PIPELINE = 'filesrc location=%s ! {src_caps}' % videosrc
        PIPELINE = f'filesrc location={videosrc} !  decodebin ! queue ! autovideoconvert ! queue ! pngenc'
        PIPELINE = f'filesrc location={videosrc} !  decodebin ! queue ! videoconvert ! videorate ! video/x-raw, framerate=30/1 ! queue '
    else:
        PIPELINE = 'v4l2src device=%s ! {src_caps}' % videosrc

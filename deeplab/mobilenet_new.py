...
MNV3_EDGETPU = {
    ...
    'block_specs': [
        ...
        ('conv_bn', 3, 1, 48,
         'relu6', None, 4, False),
        ('inverted_bottleneck', 3, 2, 96,
         'relu', None, 8, True),
        ('inverted_bottleneck', 3, 1, 96,
         'relu', None, 4, False),
        ('inverted_bottleneck', 3, 1, 96,
         'relu', None, 4, True),
        ('inverted_bottleneck', 3, 1, 96,
...
MNV3Large_BLOCK_SPECS = {
    ...
    'block_specs': [
        ...
        ('inverted_bottleneck', 3, 2, 80,
         'relu6', None, 6., False),
        ('inverted_bottleneck', 3, 1, 80,
         'relu6', None, 2.5, False),
        ('inverted_bottleneck', 3, 1, 80,
         'relu6', None, 2.3, False),
        ('inverted_bottleneck', 3, 1, 80,
         'relu6', None, 2.3, False),
        ('inverted_bottleneck', 3, 1, 112,
         'relu6', 0.25, 6., False),
...
SUPPORTED_SPECS_MAP = {
    ...
    'MobileNetV3EdgeTPU': MNV3_EDGETPU
}
...
def MobileNetV3EdgeTPU(
    width_multiplier: float = 1.0,
    output_stride: int = 32,
    bn_layer: Callable[..., Any] = tf.keras.layers.BatchNormalization,
    conv_kernel_weight_decay: float = 0.0,
    reduce_last_block_filters: bool = False,
    name: str = 'MobileNetV3EdgeTPU') -> tf.keras.Model:
  """Creates a MobileNetV3EdgeTPU model.
  Args:
    width_multiplier: A float, depth_multiplier for the whole model.
    output_stride: An optional integer specifying the output stride of the
      network.
  ...
  Returns:
    The MobileNetV3Small model as an instance of tf.keras.Model.
  """
  model = MobileNet(model_id='MobileNetV3EdgeTPU',
                    width_multiplier=width_multiplier,
                    output_stride=output_stride,
                    bn_layer=bn_layer,
                    conv_kernel_weight_decay=conv_kernel_weight_decay,
